...
| PreToolUse (git-destructive-guard) | Every Bash tool call | Hard-blocks (exit 2 — tool call never runs, not just a warning) force-push variants, remote branch deletion/mirror-push, local force branch delete, and `gh repo delete`, independent of the declarative allow/deny list in `settings.json` (which was found to not reliably block `git push --force` even with a matching deny entry). Implemented in `.claude/hooks/git-destructive-guard-hook.sh`. |
