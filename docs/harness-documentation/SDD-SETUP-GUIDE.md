...
Skills live in the harness source tree at `skills/<name>/SKILL.md` (71 skills currently). `install_globals()` in `install.sh` syncs each one into `~/.claude/skills/<name>/`, so skills are **machine-global**, not per-project — one install serves every repo.
