...
| PreToolUse (git-destructive-guard) | Every Bash tool call | Hard block (exit 2, refuses the tool call — not a soft nudge): inspects the literal command text for force-push variants (`--force`, `--force-with-lease`, `--force-if-includes`, `-f`), remote branch deletion (`--delete`, `--mirror`, empty-refspec `git push origin :branch`), local force branch delete (`git branch -D`), or `gh repo delete`. Built because the declarative allow/deny list alone was observed to not reliably block `git push --force` in some sessions. Implemented in `.claude/hooks/git-destructive-guard-hook.sh`. |
