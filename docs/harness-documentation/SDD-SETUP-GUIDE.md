...
| `/kiro:guardrails` | Audit/scaffold linter complexity **and type-evidence** rules for deterministic enforcement. Type evidence is a second, independent axis: complexity rules cap how tangled code is and say nothing about whether it kept its type information, which is where agent-written TypeScript fails most often. Reported as its own coverage line — a project can be fully compliant on complexity and score zero here |
...
| `@agents-guardrails` | `/kiro:guardrails` | Linter complexity rule auditing and scaffolding. The `audit` action now also grades **Message Quality** — for project-authored rules only (`no-restricted-imports`, `no-restricted-syntax`, `banned-api`, custom plugin rules; stock linter rules are out of scope), it flags every rule whose `message` is missing or states the violation without a fix, as a WARN-level gap, never a hard failure. The `scaffold` action fills those messages in: the primary reader of a lint violation is an agent, so every authored rule must carry **what** (which construct, which path), **why** (the constraint, one clause), and **fix** (a real path/symbol/call, not "use the correct approach"). An actionable message turns a violation into a self-correcting loop; a bare one turns it into a retry loop. The audit also covers **type-evidence rules for JS/TS** as a separate coverage line (never folded into the complexity count): stacked casts (`no-chained-type-assertions`), `unknown` in contracts (`no-unknown-parameters`/`-returns`/`-type-aliases`), untyped bags (`no-unsafe-dictionary-type`), lost inference (`no-known-value-widening`, `no-widen-then-assert`), casts with no stated invariant (`require-safety-comment-for-type-assertion`), `typeof` instead of boundary parsing (`no-runtime-typeof`), and module mocking (`no-module-mocking`). Config discovery includes `oxlint.config.*` / `.oxlintrc.json`. Scaffolding **recommends, never vendors** — the rules live in `dmmulroy/anti-slop` and are designed to be copied into the repo and edited, so the agent surfaces `npx skills add dmmulroy/anti-slop --skill install-anti-slop` and lets the human run it. Once `oxlint` is on PATH, `js-quality-gate-hook.sh` picks the rules up on every `.ts`/`.js` write with no further wiring. Two rules are judgement calls to name explicitly rather than enable silently: `require-safety-comment-for-type-assertion` (high friction, high value) and `no-runtime-typeof`'s `allowInTypeGuards`, which codebases writing real type guards should turn on |
