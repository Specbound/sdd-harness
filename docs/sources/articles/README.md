...
- Dashboard: new **Herder** section in `scripts/utils/dashboard.py` — agent-kind × model × permission-mode × prompt spawn form, plus a **responsive grid of agent cards** (`auto-fill, minmax(290px, 1fr)` — three or more per row, wrapping), each with its own status badge, token/cost figures, a one-line "what it is doing right now", and prompt/stop. Each card carries a collapsed `activity` disclosure that streams that agent's **transcript** — reasoning, tool calls, tool results, colour-coded — instead of the raw terminal, because the pane is a rendered TUI whose text would have to be pattern-matched back into meaning while the transcript already carries the events as data. The raw pane stays available behind a button as the escape hatch. Only expanded feeds poll, so watching one agent does not read every transcript on the machine every five seconds; cards rebuild only when the agent set or a status changes, so an open disclosure is not collapsed under you and a feed scrolled back is not yanked to the bottom. The **target repo is the dashboard's existing top-level repo dropdown**; the herder deliberately has no repo picker of its own, since two selectors for one concept is a thing you get wrong at 1am.
