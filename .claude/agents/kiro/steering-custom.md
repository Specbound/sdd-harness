...
- Glob(`.claude/kiro/settings/templates/steering-custom/*.md`) to find available templates
...
2. **Check if template exists**:
   - Load from `.claude/kiro/settings/templates/steering-custom/{name}.md` if available
...
4. **Generate custom steering**:
   ...
   - Apply principles from `.claude/kiro/settings/rules/steering-principles.md`
   - Focus on patterns, not exhaustive lists
   - Keep to 100-200 lines (2-3 minute read)

5. **Create file** in `.claude/steering/{name}.md`

## Available Templates

Templates available in `.claude/kiro/settings/templates/steering-custom/`:
...
## Created:
- .claude/steering/api-standards.md
