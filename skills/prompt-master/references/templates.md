...
| [M — Opus 4.7 Task Brief](#template-m--opus-47-task-brief) | Complex, multi-step, or agentic task on Claude Opus 4.7 |
| [N — JSON Prompt](#template-n--json-prompt) | Any task where ≥3 dimensions would otherwise be guessed |
...
## Template N — JSON Prompt

*Use when the user explicitly asks for a JSON prompt, when ≥3 dimensions would otherwise be guessed, or when the target is an API/programmatic consumer. Every key is a resolved dimension. Nothing left to statistical inference.*

**When to use this template:**
- User explicitly requests JSON format
- Prose request has ≥3 unspecified dimensions (tone, format, length, audience, style)
- Target is an API, pipeline, or programmatic consumer (not a chat UI)
- Output itself has defined structure (thread, report, multi-section doc)
- Task is agentic and benefits from an explicit structured brief

**Flat JSON — simple tasks:**
```json
{
  "task": "[precise verb + object]",
  "tool": "[target AI tool]",
  "tone": "[punchy / authoritative / dry / empathetic / contrarian]",
  "length": "[under X words / X sentences / X bullet points]",
  "format": "[tweet / list / paragraph / code block / JSON]",
  "audience": "[who reads this and their knowledge level]",
  "constraints": "[what MUST and MUST NOT happen]"
}
```

**Nested JSON — structured outputs:**
```json
{
  ...
  "tone": "[style modifier]",
  "structure": {
    "[section 1 name]": "[spec for section 1]",
    "[section 2 name]": "[spec for section 2]",
    "[section 3 name]": "[spec for section 3]"
  },
  "constraints": "[what to exclude or avoid]"
}
```

**API / programmatic output — lock the response schema:**
```json
{
  "task": "[analysis / classification / extraction / generation]",
  "input": "[input data or placeholder]",
  "output_format": "JSON",
  "output_schema": {
    "[field_1]": "[type and allowed values]",
    "[field_2]": "[type and allowed values]",
    "[field_3]": "[type and allowed values]"
  },
  "constraints": "respond only with valid JSON, no prose outside the object"
}
```

**Agentic JSON brief — for Claude Code, Cursor, Devin:**
```json
{
  "objective": "[single unambiguous goal]",
  "starting_state": "[what exists now — files, env, installed packages]",
  "target_state": "[what done looks like — specific files and behavior]",
  "scope": {
    "work_in": "[specific directories and files]",
    "do_not_touch": "[forbidden files and directories]"
  },
  ...
  "progress_format": "After each step output: ✅ [what was done] — [file(s) affected]"
}
```

**Rules:**
- Every key is a resolved intent dimension — no vague values ("good quality" is not a valid value)
- Use arrays for lists: `"constraints": ["no Redux", "TypeScript strict", "no new deps"]`
- Use nested objects only when the output has genuine sub-structure
- Do not nest more than 2 levels deep — beyond that, readability breaks
- For chat UIs (Claude.ai, ChatGPT): paste the JSON directly, the model will parse it
- For API calls: JSON prompt becomes the user message content; system prompt sets context separately
