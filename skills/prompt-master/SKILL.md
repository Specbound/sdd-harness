...
name: prompt-master
version: 1.7.0
description: Generates optimized prompts for AI tools. Activates only when the user explicitly asks to write, fix, improve, or adapt a prompt for a specific AI tool (LLM, Cursor, Midjourney, image AI, video AI, coding agents, etc.). Natively supports JSON-structured prompt input and generates JSON-formatted prompts when precision demands it. Does not activate for general conversation, coding tasks, document writing, or other non-prompt-engineering work.
---
...
**JSON Input Detection**: If the user writes their request as a JSON object, map keys directly to the 9 dimensions above. Keys like `task`, `tool`, `format`, `tone`, `audience`, `constraints`, `examples`, `context`, `style`, `length`, `platform`, `structure` map cleanly. Skip clarifying questions for any dimension already covered by a JSON key — the user has already specified it.
...
The model guesses when you leave dimensions unspecified. JSON eliminates the guessing surface entirely.

**Detect when to activate:**
- User writes their request as a JSON object → map keys to the 9 intent dimensions, skip clarifying questions for covered dimensions, produce the output prompt immediately
- User's prose prompt has ≥3 dimensions the model would have to guess (tone, length, format, audience, style all unspecified) AND the task is precision-sensitive → suggest JSON structure as an alternative
- Target tool is agentic (Claude Code, Cursor, Devin) with a multi-dimensional task → offer JSON structure to tighten the brief
...
```json
{
  ...
  "platform": "...",       // → Platform-specific routing
  "style": "...",          // → Style modifier
  "structure": { }         // → Nested structural spec (see below)
}
```

**Nested JSON for structured outputs:**

Use nested objects when the output itself has a defined structure — threads, reports, landing pages, multi-section docs:
...
Each nested key becomes an explicit instruction in the generated prompt — nothing left to statistical inference.

**When to suggest JSON to the user:**

If the user's prose request has ≥3 guessable dimensions AND tone/format/length/audience are all unspecified, flag it:

> "This prompt has several dimensions the model would have to guess. Here's the JSON version that eliminates the ambiguity:"

Then output both the JSON-structured version AND the generated prompt.

**When NOT to use JSON:**
- Single-dimension requests ("write a haiku about rain") — prose is fine
- Creative tasks where open space is intentional
- Conversational iteration where precision is secondary

**Generating JSON-formatted prompts:**

When the user explicitly asks for a JSON prompt, or when the target tool is API/programmatic (not a chat UI), output the prompt in JSON format. Read Template N from references/templates.md for the full JSON Prompt template.
...
- **Prose with ≥3 unspecified dimensions** → model fills gaps with statistical average. Flag: "Tone, length, and format are unspecified — here's the JSON version that locks all three." Offer JSON structure as alternative.
...
**JSON structure** — for precision-sensitive tasks with ≥3 guessable dimensions, or when the target is an API/programmatic consumer. Every key is a resolved dimension; every nested object is explicit structure. Eliminates statistical gap-filling.

---

### Agentic Output Warning

For prompts targeting agentic tools (Claude Code, Devin, Cursor, Windsurf, Cline, Bolt, SWE-agent, Manus, or anything that executes commands or edits files — mandatory for Templates G, H, M, N and any prompt referencing filesystem, terminal, dependency, or database operations), append this notice:
...
6. If the user's request had ≥3 guessable dimensions, was JSON structure offered?
...
| File | Read When |
|------|-----------|
| [references/templates.md](references/templates.md) | You need the full template structure for any tool category, including Template N (JSON Prompt) |
