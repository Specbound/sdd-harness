...
**Scale scrutiny to blast radius, not nesting depth.** Depth of subagent delegation is not the risk signal — graph position is. For any agent-generated artifact, ask "what else becomes wrong if this artifact is wrong?" A leaf node feeding one downstream consumer can tolerate a light post-condition. A node whose output fans out to many consumers (a plan several executors follow, a classification that routes everything after it) deserves a proportionally stronger gate: a dedicated verifier, a structured/schema-constrained output, the original evidence traveling alongside the conclusion (not just a summary of it), N-way independent production before the result is trusted, or human approval before it's allowed to fan out further. A 5-layer graph of narrow, low-fan-out nodes can be more reliable than a 2-layer graph where the first hop makes a broad judgment everything else blindly inherits.
