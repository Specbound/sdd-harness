...
When a multi-agent system dispatches work to specialized agents, the routing layer itself is a design surface. Four requirements distinguish auditable routing from ad-hoc dispatch.

### Requirements

| Requirement | Description | How to verify |
|---|---|---|
| **Specificity** | Each agent has explicit capability scope — what it handles AND what it refuses | Agent's system prompt includes an explicit "Do not handle:" section |
| **Selectivity** | Routing decisions are consistently correct — the right agent for the right task | Track routing accuracy: correct_routes ÷ total_routes per agent per session |
| **Composability** | Agent A's output format meets agent B's input contract — no implicit translation | Define input/output schemas per agent; test handoffs with schema validation |
| **Verifiability** | Post-condition checks run after execution, before downstream consumption | Every `agent()` call followed by a deterministic check before results flow on |

### Post-Condition Coupling Pattern

Never let a routing decision be the last check. Couple each skill invocation to a post-condition:

```python
def route_and_verify(task, agent_registry, post_conditions):
    """Route a task and verify the result before passing downstream."""
    agent = agent_registry.select(task)
    result = agent.run(task)

    condition = post_conditions.get(agent.name)
    if condition and not condition(result):
        raise RoutingFailure(
            f"Post-condition failed for {agent.name}: {condition.description}"
        )
    return result
```

The post-condition can be lightweight: schema check, assertion, test, confidence threshold. The key is determinism — not model self-assessment.

### Routing Audit Trail

Log routing decisions as structured events to make failures diagnosable:

```python
routing_event = {
    "task_id": task.id,
    "selected_agent": agent.name,
    "selection_reason": agent.match_score(task),  # why this agent won
    "post_condition": "passed" if passed else f"failed: {failure_msg}",
    "timestamp": now(),
}
```

This separates two failure modes that are otherwise easy to conflate:
- **Wrong route taken** — correct agent was available but not selected (selectivity failure)
- **Correct route, bad output** — right agent selected, post-condition caught the bad result (verifiability catches this)
