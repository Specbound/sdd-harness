...
- **Independence-budgeting**: a finding is only as trustworthy as the independence of whoever confirmed it. The orchestrator's own self-review is capped at *low* confidence (it shares the model's blind spots); confidence rises only with agreement from independent reviewers.
  - *A concrete template for this*: cursor/plugins' "show-me-your-work" skill operationalizes independence-budgeting with two artifacts. (1) An append-only TSV decision log — columns `timestamp, phase, decision, rationale, evidence-pointer, result` — written incrementally as the agent works, one line per decision ("one line or it's not crisp yet"; no paragraph justifications), where the evidence-pointer column must be a concrete reference (commit SHA, file path, test-run result) and never prose. (2) A cross-model review gate before the work is declared complete: a different model family than the one that did the work reviews the decision log and flags any decision with weak or missing evidence in an "Attention" section. The log is the artifact; the cross-model review is the independence check. Offer this as an optional pattern when a verify skill needs to log and defend its own decisions autonomously — not a mandatory format.
