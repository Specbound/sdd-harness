...
> **Scope:** This skill covers the **micro** layer — grading individual runs/responses (LLM-as-judge, rubrics, quality gates). For the **macro** layer — discovering recurring failure *patterns* across many traces and ranking them by impact — see the `macro-evals` skill. Micro signal feeds macro discovery.
...
### Automated Grade-and-Revise Loops

When your rubric targets a **single agent's iterative output** (research briefs, documents, reports), the Claude Managed Agents Outcomes feature can run the grader automatically in a loop — no custom orchestration needed. See the `cma-outcomes` skill for:

- Writing rubrics that force the grader to fetch and verify evidence
- `user.define_outcome` event API and event streaming patterns
- Full Python template for writer + grader sessions
- Citation verification (LIVE / VERBATIM / SUPPORTS_CLAIM triplet)

Use `evaluation` for framework design; use `cma-outcomes` when you're ready to implement the loop with the CMA API.
...
- [[rl-agent-training]] — for RULER (comparative LLM-as-judge reward with no labeled data); use when the evaluation signal is also the RL training signal, not just offline measurement
