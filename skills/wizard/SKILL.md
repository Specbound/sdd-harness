...
This is the generic form of a pattern this harness already builds one-off instances of by hand (`scripts/setup/headroom-setup.sh`, `gitnexus-setup-agent`'s interview-and-provision flow) — use it instead of writing a bespoke setup script from scratch next time.
...
- `environment-setup-guide` — general dev-environment onboarding; use `wizard` specifically when a step requires a human to click through a third-party UI, not just run commands.
- `gitnexus-setup-agent` — an existing hand-written instance of this exact pattern (interview → provision → configure); a future rewrite of it could use this template instead of bespoke prompting.
