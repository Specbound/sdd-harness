...
### ❌ Memory blocks over 500 characters
Inline prompt memory (hot-memory, system-prompt blocks) that exceeds ~500 chars per block starts crowding out working context. Keep behavioral/preference state tightly bounded; push data to files.

## Which Type, When

| Information type | Use |
|---|---|
| Preferences, behavior, identity, character | Memory block (inline in prompt, < 500 chars) |
| Data, knowledge, facts | Files with hierarchical paths |
| Procedures, workflows, how-to | Skills (indexed files with SKILL.md) |
