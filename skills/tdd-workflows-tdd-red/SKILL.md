...
0. **Source scan first (grounding step)**
   - Bug fix: run `git diff HEAD` (or read the failing test / bug report) to see the exact broken path
   - New feature: read the spec + any existing interfaces (function signatures, endpoints, data shapes)
   - Map what actually exists: real parameter names, real return types, real branch conditions
   - List only test scenarios that correspond to real code paths or explicitly specified behaviors
   - Mark anything assumed (not yet in code or spec) with `# TODO: verify path exists` — write it as a test but flag it
   - **Stop** if you can't ground a test in source or spec — ask for the spec before proceeding

1. Identify behaviors, constraints, and edge cases — from the source scan, not from assumptions.
