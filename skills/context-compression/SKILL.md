...
### Compaction Modes and Boundary Timing

Three operational modes exist for triggering compaction in agent systems:

| Mode | Trigger | Best Use |
|------|---------|----------|
| **Automatic** | SDK/runtime manages based on context pressure | Default for production; least configuration |
| **Threshold-based** | Explicit context-size threshold (e.g. 70% utilization) | Predictable environment policies |
| **Forced checkpoint** | Manually triggered at phase boundaries | Multi-phase workflows, small datasets |

**The boundary timing principle:** "Compact at meaningful workflow boundaries, not after every turn."

Phase boundaries are natural compaction points because the working state at that moment is coherent and complete — the agent has just finished one phase and hasn't started the next. Compacting mid-turn risks losing partial reasoning chains or in-flight decisions.

**What to preserve at compaction:**
...
- Strip file paths, function names, or error messages
- Collapse multi-step decisions into vague summaries
- Lose specific values (numbers, identifiers) needed in subsequent phases
