...
## Step 0: RAG vs CAG Decision Gate

**Before designing anything**, run through this decision to pick the right paradigm.

Ask the user (or infer from context) — answer YES/NO to each:

| Question | YES → CAG | NO → RAG |
|----------|-----------|----------|
| Does the entire knowledge base fit in one model's context window? (≤ 128K tokens, roughly ≤ 100 pages) | ✓ | |
| Is the knowledge static or updated less than once per day? | ✓ | |
| Is minimizing per-query latency a hard requirement? | ✓ | |
| Is a local HuggingFace model available (not API-only)? | ✓ | |

**Decision rule:**
- **3 or more YES** → Invoke `cag-implementation` skill instead of continuing here.
- **2 or fewer YES** → Continue with the RAG architecture below.

> **Why this matters:** CAG eliminates retrieval latency and system complexity entirely for bounded, stable knowledge. Using RAG for a 50-page knowledge base is engineering overhead you don't need. Conversely, CAG breaks down at large corpora or when knowledge changes frequently.
