...
| **Conflicting instructions** | Two lines (within the same file, or across CLAUDE.md/AGENTS.md/active skills) that pull in opposite directions on the same decision — e.g. "leave documentation as appropriate" vs. "never add comments" → flag both, propose which should win or how to scope them apart. | needs-update if the conflict is load-bearing (would change agent output); minor if cosmetic |
