...
| `orchestration/setup-mac-orchestrator.sh` | Register `daily-orchestrator.sh` with launchd (macOS). Wraps the run in `caffeinate -i` so the LaunchAgent's unattended 18:00 fire isn't cut short by idle/display sleep. |
