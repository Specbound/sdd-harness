...
- `~/.claude/skills/` via the Skill tool (including `macro-evals`)
...
2. Follow the `macro-evals` skill's five-phase methodology exactly. Ground every finding in real `run_id`/`span_id` values from `query_traces`.
